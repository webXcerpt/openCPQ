...
class TableType extends Type {
	constructor(options, columns) {
		...
		super("table", function makeTable(list = defaultValue, updateTo, ctx) {
			var rows = list.map((element = {}, i) => {
				function updateElement(newElement) {
					var newList = list.slice();
					newList[i] = newElement;
					updateTo(newList);
				}
				return _this.makeRow(element, updateElement, {
						...ctx, rowIndex: i, path: ctx.path.ext(i)
				});
			});
			...
			return new TableNode({columns, rows, list, splice});
		});
	...
	makeRow(element, updateElement, ctx) {
		return new GroupNode([
			for ({name, type} of this._columns)
				type.makeNode(
					element[name],
					newValue => updateElement({...element, [name]: newValue}),
					ctx
				)
		]);
	}
