...
function CSelect(rawCases) {
	return new Type("select", function makeSelect(value, updateTo, ctx) {
		...
		messages = [for (m of messages) ctx.problems.add(m)];
		var detailNode = currentCase.type.makeNode(detail, updateDetail, ctx);
		return new SelectNode({cases, caseName, userSelected, retract, mode, messages, detailNode, updateCase});
	});
}
...
class SelectNode extends Node {
	...
	renderHB() {
		var {cases, caseName, userSelected, retract, mode, messages, detailNode, updateCase} = this.__options;
		var head = renderWithValidation(
			<span>
				<select
					className={`select-select select-mode-${mode}`}
					value={caseName}
					onChange={e => updateCase(e.target.value)}
				>
					{[for ({name, label, mode} of cases)
						// Display an "hidden" option nevertheless if it is selected:
						if (mode !== "hidden" || name == caseName)
							<option className={`select-option option-mode-${mode}`} key={name} value={name}>{label}</option>
					]}
				</select>
		...
		var {head: detailHead, body} = detailNode.renderHB();
		if (detailHead != undefined)
			head = <HBox>{head}{detailHead}</HBox>;
		return {head, body};
	}
}
...
function CEither(rawOptions = {}, thenType, elseType) {
	return new Type("either", function makeEither(value = {}, updateTo, ctx) {
		...
		var detailNode = detailType.makeNode(detail, updateDetail, ctx);
