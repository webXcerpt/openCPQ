...
function CLinearAggregation(name, class_, type) {
	return new Type("linearAggregation", function makeLinearAggregation(value, updateTo, ctx) {
...
function CMultiplying(factor, type) {
	return new Type("multiplying", function makeMultiplying(value, updateTo, ctx) {
		var subCtx = {...ctx};
		var {linearAggregators = []} = ctx;
		linearAggregators.forEach(k => subCtx[k] = ctx[k].multiplying(factor));
		var node = type.makeNode(value, updateTo, subCtx);
