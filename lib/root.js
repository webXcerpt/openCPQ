...
var RootWidget = React.createClass({
	...
	render() {
		...
		return <div style={{height: "100%"}}>
			<div style={{height: "5%", backgroundColor: "black", display: "flex", verticalAlign: "center"}}>
				<div style={{marginTop: "auto", marginBottom: "auto", marginLeft: "3%"}}>
					<button disabled={past  .length === 0} onClick={this._undo}>undo</button>
					<button disabled={future.length === 0} onClick={this._redo}>redo</button>
					<button disabled={past  .length === 0} onClick={this._reset}>reset</button>
					{// local storage is not always available (e.g., in IE with file: URLs)
						(window.localStorage == undefined) ? undefined : [
							<button onClick={this._save}>save</button>,
							<button disabled={localStorage.openCPQ == undefined} onClick={this._restore}>restore</button>
						]
					}
					<input id="import" type="file" name="files"/>
					<button onClick={this._import}>import</button>
					<button disabled={this.state.now == undefined} onClick={this._export}>export</button>
				</div>
			</div>
			<div style={{height: "95%"}}>
				{packed(node.renderHB())}
			</div>
		</div>;
	}
});
