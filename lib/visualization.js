...
function CWrapVisualization(renderWrapper, type) {
	return new Type("visualizationWrapper", function makeVisualizationWrapper(value, updateTo, ctx) {
		var subVisualization = new VisualizationContainer(renderWrapper);
		ctx.visualization.add(subVisualization);
		return type.makeNode(value, updateTo, {...ctx, visualization: subVisualization});
	});
}
